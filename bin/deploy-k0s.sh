...
function usage()
{
    ...
    echo "This script will deploy K0s Kubernetes cluster on the local machine or another host" >&2
...
export leaf_target_path="clusters/k0s/$location-$cluster_name"
...
echo "export public_address=\"${public_address}\"" >> /tmp/${location}-${cluster_name}-env.sh
...
if [ "${hostname}" != "localhost" ]; then
  $scp_cmd -r ${utils_dir}/k0s-leafs ${username_str}${hostname}:/tmp >/dev/null
  
  $scp_cmd -r /tmp/${location}-${cluster_name}-env.sh ${username_str}${hostname}:/tmp/env.sh >/dev/null

  # $scp_cmd -r $(local_or_global resources/k0s.yaml) ${username_str}${hostname}:/tmp >/dev/null
  $scp_cmd -r $(local_or_global resources/audit.yaml) ${username_str}${hostname}:/tmp >/dev/null

  if [ -n "$install" ]; then
    $ssh_cmd ${username_str}${hostname} "source /tmp/k0s-leafs/leaf-install.sh $debug_str"
  fi

  $ssh_cmd ${username_str}${hostname} "source /tmp/k0s-leafs/leaf-deploy.sh $debug_str"

  $scp_cmd ${username_str}${hostname}:.kube/config ~/.kube/${hostname}-${cluster_name}.kubeconfig >/dev/null
  ...
  cp -r ${utils_dir}/k0s-leafs /tmp
  cp -r $(local_or_global resources/k0s.yaml) /tmp
  cp -r $(local_or_global resources/audit.yaml) /tmp

  if [ -n "$install" ]; then
    ${utils_dir}/k0s-leafs/leaf-install.sh $debug_str
  fi
...
if [ -z "$mgmt" ]; then
  export target_path="clusters/k0s/$hostname-$cluster_name"
fi
...
export cluster_name
export hostname
export wge_cluster_name="k0s-${hostname}-${cluster_name}"
git pull
export type=k0s
cat $(local_or_global resources/leaf-flux.yaml) | envsubst > clusters/k0s/$hostname-$cluster_name/flux/flux.yaml
git add clusters/k0s/$hostname-$cluster_name/flux/flux.yaml

if [[ `git status --porcelain` ]]; then
  git commit -m "deploy kustomizations to apply WGE SA to k0s cluster $hostname-$cluster_name"
  git pull
  git push
fi
...
git pull
mkdir -p clusters/k0s/$hostname-$cluster_name/wge
cat $(local_or_global resources/leaf-wge.yaml)| envsubst > clusters/k0s/$hostname-$cluster_name/wge/flux.yaml
git add clusters/k0s/$hostname-$cluster_name/wge/flux.yaml

if [[ `git status --porcelain` ]]; then
  git commit -m "deploy kustomizations to apply addons and apps to k0s cluster $hostname-$cluster_name"
  git pull
  git push
fi
...
cat $(local_or_global resources/wge-kubeconfig.yaml) | envsubst > /tmp/k0s-${hostname}-${cluster_name}-wge-kubeconfig.yaml
vault kv put -mount=secrets/leaf-cluster-secrets k0s-${hostname}-${cluster_name}  value.yaml="$(cat /tmp/k0s-${hostname}-${cluster_name}-wge-kubeconfig.yaml)"

mkdir -p clusters/management/clusters/k0s/$hostname-$cluster_name
cat $(local_or_global resources/mgmt-flux.yaml) | envsubst > clusters/management/clusters/k0s/$hostname-$cluster_name/flux.yaml
git add clusters/management/clusters/k0s/$hostname-$cluster_name/flux.yaml
if [[ `git status --porcelain` ]]; then
  git commit -m "deploy kustomization to apply kubeconfig and gitopsCluster for k0s cluster $hostname-$cluster_name"
  git pull
  git push
fi
